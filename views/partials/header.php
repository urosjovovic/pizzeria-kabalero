<!-- <nav class="container-fluid navbar navbar-expand-md navbar-light bg-white p-0 invisible navbar-content-pusher">
  <div class="container p-0">
    <a class="navbar-brand p-0 pt-1 ms-3 ms-lg-0" href="/#">
      <img src="images/logo.svg" alt="Picerija Favola Logo" />
    </a>
    <h1 class="d-block d-md-none me-3 mb-0">Pizerija Favola</h1>
    <button class="navbar-toggler me-3 me-lg-0" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-3 mb-2 mb-lg-0 ms-3">
        <li class="nav-item">
          <a class="nav-link" href="/#pice">Pice</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#tortilje">Tortilje</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#piroske-i-obroci">Piroške i obroci</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#sendvici">Sendvici</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#palacinke-slane">Palačinke Slane</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#palacinke-slatke">Palačinke Slatke</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#dodaci">Dodaci</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#pica">Piće</a>
        </li>
        <li class="nav-item free-dostava-item">
          <a class="nav-link free-dostava-btn d-md-block d-none py-1" href="/#zone">DOSTAVA</a>
        </li>
        <div class="d-md-none py-2 border-info border-top">
          <span>Telefon: <a class="ms-auto" href="tel:+38520478708">+385 20 478 708</a>
          </span>
          <p class="radno-vreme mb-0">Radno vreme - Pon-Sub: 12:00-03:30h</p>
          <p class="radno-vreme mb-0">Adresa: Ljubićka 11, Beograd</p>
        </div>
      </ul>
      <div class="call-to-action d-none d-md-block">
        <a class="btn ms-auto" href="tel:+38520478708">+385 20 478 708</a>
        <span class="radno-vreme">Pon-Sub: 12:00-03:30h</span>
      </div>
    </div>
  </div>
</nav> -->
<nav class="container-fluid navbar navbar-expand-md navbar-light bg-white p-0">
  <div class="container p-0">
    <a class="navbar-brand p-0 pb-1 pt-1 ms-3 ms-lg-0" href="/#">
      <!-- <img loading="lazy" alt="Picerija Favola Logo" src="images/favola-header-logo.png"> -->
      <img src="images/logo.svg" alt="Pizzeria Kabalero Logo" />
    </a>
    <h1 class="d-block d-md-none me-3 mb-0 text-center">Pizzeria </br> Kabalero</h1>
    <button class="navbar-toggler me-3 me-lg-0" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-3 mb-2 mb-lg-0 ms-3">
        <li class="nav-item">
          <a class="nav-link" href="/#cold-appertizers">Cold Appertizers</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#salads">Salads</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#soups">Soups</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#hot-side-dishes">Hot Side Dishes</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#pasta-dishes">Pasta Dishes</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#tagliatele">Tagliatele</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#dodaci">Dodaci</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/#pica">Piće</a>
        </li>
        <li class="nav-item free-dostava-item">
          <a class="nav-link free-dostava-btn d-md-block d-none py-1" href="/#zone">Directions</a>
        </li>
        <div class="d-md-none py-2 border-info border-top">
          <span>Phone: <a class="ms-auto" href="tel:+38520478708">+385 20 478 708</a>
          </span>
          <p class="radno-vreme mb-0">Every day: 10:00-00:00</p>
          <p class="radno-vreme mb-0">Address: Put Tihe 14, Cavtat</p>
        </div>
      </ul>
      <div class="call-to-action d-none d-md-block">
        <a class="btn ms-auto" href="tel:+38520478708">+385 20 478 708</a>
        <span class="radno-vreme">Every day: 10:00-00:00</span>
      </div>
    </div>
  </div>
</nav>
<nav class="mobile-bottom-bar navbar fixed-bottom navbar-light bg-light justify-content-center d-md-none pb-3 pt-0">
  <div class="w-100 text-center border-bottom border-1 text-muted">Address: Put Tihe 14, Cavtat</div>
  <div class="quick-info mx-3">
    <p class="besplatna-dostava my-0">
      <b>Pay by card or cash</b>
    </p>
    <p class="radno-vreme my-0">Every day: 10:00-00:00</p>
  </div>
  <a class="nav-item mx-3" target="_blank" href="https://www.google.com/maps/dir//pizzeria+kabalero/data=!4m6!4m5!1m1!4e2!1m2!1m1!1s0x134c0d28da64fed9:0xcfae2f1cb32c9c25?sa=X&ved=1t:3061&ictx=111" aria-label="Lokacija">
    <i class="directions-icon fa-solid fa-diamond-turn-right fa-2xl"></i>
  </a>
  <a class="nav-item mx-3" target="_blank" href="https://maps.app.goo.gl/oBfNNK2wroz3GdESA" aria-label="Lokacija">
    <i class="location-icon fa-solid fa-location-dot fa-2xl"></i>
  </a>
  <a class="nav-item mx-3" href="tel:+38520478708" aria-label="Phone">
    <i class="phone-icon fa-solid fa-phone fa-2xl"></i>
  </a>
</nav>
<div class="page-main-content">
